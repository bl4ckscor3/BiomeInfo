...
public class BiomeInfoRenderer {
	...
	public static void onResourceManagerReload(RegisterClientReloadListenersEvent event) {
		event.registerReloadListener((barrier, manager, backgroundExecutor, gameExecutor) -> CompletableFuture.runAsync(NAME_CACHE::clear, backgroundExecutor).thenCompose(barrier::wait));
	}
